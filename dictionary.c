...
//Agrega el atributo a la entidad a la que pertenece
...
//Muestra las entidades con los apuntadores
...
//Captura un atributo para una entidad
...
//Muestra los atributos de la entidad
...
ATTRIBUTE searchAttributeByName(FILE* dataDictionary,long attributesPointer, const char* attributeName){
    long currentAttributeDirection = attributesPointer;
    ...
    while (currentAttributeDirection != EMPTY_POINTER) {
        ...
        fseek(dataDictionary, currentAttributeDirection, SEEK_SET);
        fread(currentAttribute.name, DATA_BLOCK_SIZE, 1, dataDictionary);
        fread(&currentAttribute.isPrimary, sizeof(long), 1, dataDictionary);
        fread(&currentAttribute.type, sizeof(long), 1, dataDictionary);
        fread(&currentAttribute.size, sizeof(long), 1, dataDictionary);
        ...
        currentAttributeDirection = currentAttribute.nextAttribute;
    }
...
//Funcion auxiliar para eliminar una entitdad
void deleteEntity(FILE* dataDictionary){
    ...
    fseek(dataDictionary, MAIN_ENTITY_POINTER, SEEK_SET);
    fread(&nextEntity, sizeof(long), 1, dataDictionary);
    ...
    while (nextEntity != EMPTY_POINTER) {
        ...
        fseek(dataDictionary, currentEntityDirection, SEEK_SET);
        currentEntityDirection = ftell(dataDictionary); 
        fread(currentEntity.name, DATA_BLOCK_SIZE, 1, dataDictionary);
        fread(&currentEntity.dataPointer, sizeof(long), 1, dataDictionary);
        fread(&currentEntity.attributesPointer, sizeof(long), 1, dataDictionary);
        fread(&currentEntity.nextEntity, sizeof(long), 1, dataDictionary);
        ...
        if (strcmp(currentEntity.name, entityName) == 0) {
            ENTITY resultEntity = removeEntity(dataDictionary, currentEntityDirection, entityName);
            printf("\nEntity '%s' deleted.\n", currentEntity.name);
            return resultEntity; 
        }

        nextEntity = currentEntity.nextEntity;
    }
...
//Eliminar un atributo de una entidad 
...
void entityMenu(FILE* dataDictionary) {
    ...
    do {
        ...
        switch (option) {
            case 1:
                captureEntities(dataDictionary);//Funciona correctamente
                break;
            case 2:
                deleteEntity(dataDictionary); ///cheacar el funcionamiento
                break;
            case 3:
                //modifyEntity(dataDictionary); //checar el funcionamento 
                break;
            case 4:
                showEntities(dataDictionary);//Funciona correctamente 
                break;
            case 0:
                printf("\nReturning to Entityes and Attribute Menu...\n");
                selectionEntitiesAttributes(dataDictionary); //Funciona correctamente
                break;
            default:
                printf("\nInvalid option. Try again.\n");
        }
...
void attributeMenu(FILE* dataDictionary) {
    ...
    do {
        ...
        switch (option) {
            case 1:
                captureAttributesForEntity(dataDictionary); //checar por que no agrega el atributo a la entidad
                break;
            case 2:
                //deleteAttribute(dataDictionary);  // AFunciona correctamente 
                break;
            case 3:
                //modifyAttribute(dataDictionary); //cheacar el funcionamiento
                break;
            case 4:
                showEntitiesWithAttributes(dataDictionary);  //Muestra los atributos corrrectamente los atributos
                break;
            case 0:
                selectionEntitiesAttributes(dataDictionary); //Funciona correctamente
...
//Menu de menus (Entidades/Atributos)
